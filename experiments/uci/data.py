...
# modified from: https://github.com/hughsalimbeni/bayesian_benchmarks/blob/a8fac6f285e700e29353208fb9828e7aa224aa4b/bayesian_benchmarks/data.py
...
# EDIT: changed to fix the data leakage from training to test
def normalize(X, mean=None, std=None):
    X_mean = np.average(X, 0)[None, :] if mean is None else mean
    X_std = 1e-6 + np.std(X, 0)[None, :] if std is None else std
...
class Dataset(object):
    def __init__(self, split=0, prop=0.9):
        ...
        # EDIT: changed to fix the data leakage from training to test
        self.X_train, self.Y_train = self.preprocess_data(X_raw[ind[:n]], Y_raw[ind[:n]])
        self.X_test, self.Y_test = self.preprocess_data_test(X_raw[ind[n:]], Y_raw[ind[n:]])
    ...
    def preprocess_data_test(self, X, Y):
        X, _, _ = normalize(X, self.X_mean, self.X_std)
        Y, _, _ = normalize(Y, self.Y_mean, self.Y_std)
        return X, Y
...
# Edit:
# From 'Semi-supervised Deep Kernel Learning: Regression with Unlabeled Data by Minimizing Predictive Variance'
# (https://arxiv.org/abs/1805.10407). They have made the data available at the following link:
# https://www.dropbox.com/sh/o5nruvv6g4x8bzn/AAAnyHRcQpW1BAUxntQJ_4kVa?dl=0
@add_regression
class CTSlice(Dataset):
    N, D, name = 53500, 384, 'ctslice'
    url = None

    @property
    def datapath(self):
        return os.path.join(DATA_PATH, 'ctslice')
    ...
    def read_data(self):
        X = np.load(self.datapath + '/X.npy')
        y = np.load(self.datapath + '/y.npy')
        return X, y.reshape(-1, 1)

@add_regression
class Buzz(Dataset):
    N, D, name = 583250, 77, 'buzz'
    url = None

    @property
    def datapath(self):
        return os.path.join(DATA_PATH, 'buzz')
...
def split_train(data, val_size=0.2):
...
def get_data_loaders(X_train, y_train, X_val, y_val, X_test, y_test,
                     batch_size, test_batch_size, shuffle=True, num_workers=4, num_samples_train=None):
    X_train, y_train = torch.from_numpy(X_train), torch.from_numpy(y_train).view(-1)
    X_test, y_test = torch.from_numpy(X_test), torch.from_numpy(y_test).view(-1)

    # sample data in case needed
    if num_samples_train is not None:
        train_ind = torch.sort(torch.argsort(torch.rand(X_train.shape[0], device=X_train.device))
                               [:num_samples_train])[0]
        X_train = X_train[train_ind, :]
        y_train = y_train[train_ind]

    train_data = TensorDataset(X_train.double(), y_train.double())
    test_data = TensorDataset(X_test.double(), y_test.double())
    ...
    if y_val is not None:
        X_val, y_val = torch.from_numpy(X_val), torch.from_numpy(y_val).view(-1)
        val_data = TensorDataset(X_val.double(), y_val.double())
...
def get_splitted_data_loaders(X_train, y_train, X_val, y_val, X_test, y_test,
                              batch_size, test_batch_size, shuffle=False, num_workers=4, pct=0.5):
    X_train, y_train = torch.from_numpy(X_train), torch.from_numpy(y_train).view(-1)
    X_test, y_test = torch.from_numpy(X_test), torch.from_numpy(y_test).view(-1)

    num_samples = int(X_train.shape[0] * pct)  # number of samples to learn the posterior over Zs
    train1_ind = torch.sort(torch.argsort(torch.rand(X_train.shape[0], device=X_train.device))[:num_samples])[0]
    train2_ind = torch.as_tensor([i for i in range(X_train.shape[0]) if i not in train1_ind], device=X_train.device)

    X_train1, X_train2 = X_train[train1_ind, ...], X_train[train2_ind, ...]
    y_train1, y_train2 = y_train[train1_ind, ...], y_train[train2_ind, ...]

    train1_data = TensorDataset(X_train1.double(), y_train1.double())
    train2_data = TensorDataset(X_train2.double(), y_train2.double())
    train_data = TensorDataset(torch.cat((X_train1.double(), X_train2.double())),
                               torch.cat((y_train1.double(), y_train2.double())))
    test_data = TensorDataset(X_test.double(), y_test.double())
