...
from experiments.utils import to_one_hot, detach_to_numpy
import logging
...
class ECELoss(nn.Module):
    ...
    def forward(self, probs, labels, path=None, color='darkblue'):
        ...
        y_one_hot = to_one_hot(targets, num_classes=num_classes, dtype=probs.dtype)
        bri = (torch.norm(probs - y_one_hot, dim=1) ** 2).mean()
        #bri = torch.tensor([0.0])
